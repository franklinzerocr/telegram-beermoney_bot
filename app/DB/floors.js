async function getNewlyCreatedFloors(dbConnection) {
  try {
    ...
    let result = await dbConnection.query('SELECT * FROM floor f WHERE DateTime IS NOT NULL AND ((Level=0 AND ' + timeConditionEntry + ') OR (Level=-4 AND ' + timeConditionEnd + ' )) AND TelegramID is Null AND OrderID>0 AND Pair NOT LIKE  "BNB" ');
    return result;
  } catch (e) {
    console.log(e);
...
async function getNewlyCreatedFloorsSignals(dbConnection) {
  try {
    ...
    let result = await dbConnection.query('SELECT * FROM floor f WHERE DateTime IS NOT NULL AND ((Level=0 AND ' + timeConditionEntry + ') OR (Level=-4 AND ' + timeConditionEnd + ' )) AND SignalsID is Null AND OrderID>0 AND Pair NOT LIKE "BNB" ');
    return result;
  } catch (e) {
    console.log(e);
    console.log('getNewlyCreatedFloor Error');
